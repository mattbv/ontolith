...
- **Integer**/**Float**: a dedicated regex (`[+-]?[0-9]+` / `[+-]?(?:[0-9]+\.[0-9]*|\.[0-9]+|[0-9]+)(?:[eE][+-]?[0-9]+)?`), not bare `int()`/`float()` — those accept PEP-515 underscore separators, surrounding whitespace, and (for `int()`) non-ASCII decimal digits, none of which round-trip consistently through the KI-039 SQL `CAST`/`TRY_CAST` paths this validation exists to back up (verified empirically during review: SQLite casts `"5_000"` to `0`, DuckDB to `5000` — a validation-passing value that would still diverge cross-backend under the looser stdlib parse). `float()`'s bare form also accepts `"inf"`/`"nan"`, excluded by the regex too.
...
- **Date**/**DateTime**: `date.fromisoformat(value)`/`datetime.fromisoformat(value)` — Python's `fromisoformat` grammar, which is close to but not exactly ISO 8601 in both directions (e.g. accepts a space date/time separator, doesn't accept ordinal dates). `Date` additionally rejects any string carrying a time component. Its accepted set is a strict *subset* of `DateTime`'s (every valid `Date` string is also a valid `DateTime` string), not disjoint from it — a `Date`-typed predicate still only ever compares against `date.fromisoformat`, so this doesn't weaken the per-predicate check itself, but the two types' grammars aren't mutually exclusive.
...
- **JSON**: `json.loads(value)`, rejecting on any `ValueError` (covers `JSONDecodeError`) — any valid JSON document (object, array, string, number, bool, `null`), matching this ADR's own type table rather than restricting to objects. `parse_constant` rejects Python's non-standard `NaN`/`Infinity`/`-Infinity` extensions too, since RFC 8259 doesn't define them and the stdlib parser otherwise accepts them silently.
