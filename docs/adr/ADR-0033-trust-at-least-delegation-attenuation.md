...
**Related**: SPEC §8.4 (effective capability under delegation), `govern/policy.py`'s `ThresholdPolicy.evaluate()`, KI-028 (push-down design), KI-036 (bitemporal `as_of` scoping), KI-037 (`candidate_ids` narrowing), KI-039 (prior cross-backend `min`/`least`-shaped SQL divergence), KI-047
