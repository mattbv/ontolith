...
- **The eligible-winner invariant this ADR establishes holds only at creation time, not durably.** `flag_contradiction()` guarantees a *new* contradiction starts with at least one eligible member, but nothing stops that last-remaining eligible member from being independently `retract()`-ed afterward, eroding an already-open contradiction back down to zero eligible winners the same way a pre-fix `flag_contradiction()` call could have produced directly. `member_ids` is a strictly-growing union on every write path that touches it (both `flag_contradiction()`'s own extend branch and conflict-routing's own contradiction-extension branch), so nothing *removes* eligibility either — this is a gap in what `retract()` itself guards, not something this ADR's check can or should prevent. See KI-051 (`retract()` on a contradiction member whose status isn't `flagged` bypasses both the party guard and the capability floor) for the related, still-open gap in that direction.
