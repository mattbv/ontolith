...
- ~~KI-059 (MCP error-code drift from REST/GraphQL) is not addressed by this ADR — `ontolith.retract` reuses the same `auth_error`/`capability_error`/`not_found`/`validation_error` codes `ontolith.resubmit` already uses, matching existing (drifted) MCP convention rather than introducing yet another inconsistency, but not fixing the underlying drift either.~~ **Resolved by KI-059 itself**: `ontolith.retract`'s codes now come from `exc.code`/the exception class's own `.code` attribute, same as every other tool in `interfaces/mcp.py` — no ADR-0039-specific change was needed, the fix landed at the shared `mcp.py` level this ADR's tool already sits inside.
