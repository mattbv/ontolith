...
**Why `Validator.validate()`'s `kb` parameter is now typed `ValidatorKbView`, not `ReadOnlyView`:** `Ontology` needs `Validator`'s type to type-hint its own constructor parameters, but `plugins/ports.py` already imports `ReadOnlyView`/`WriteView` from `plugins/views.py`, which imports `Ontology` — a literal import cycle if `ontology.py` imported `plugins/ports.py` at runtime. `ValidatorKbView` is a minimal structural `Protocol` (`get_entity`/`assertions`/`query`/`as_of`) that both `ReadOnlyView` and `Ontology` already satisfy without any inheritance or runtime import — `ontology.py` only needs `Validator`'s type under `TYPE_CHECKING`, which mypy resolves statically without ever executing the cycle at runtime. This is the exact same shape `govern/policy.py`'s `KbView` already uses, for the identical reason (KI-017, ADR-0025) — not a new pattern, a second application of an established one. SPEC §13.2's literal signature (`def validate(self, a: Assertion, kb: ReadOnlyView) -> list[Violation]`) is not updated to match — ADR-0025 already established the precedent of leaving the SPEC's `PolicyStrategy.evaluate(..., kb: ReadOnlyView)` signature as-is while the actual `kb` parameter is typed `KbView`, on the grounds that `ReadOnlyView` was "the wrong type for this purpose, independent of any [circular-import] concern"; the same reasoning applies here without needing to re-litigate it.

**Why a validator rejecting `propose`/`propose_ref`'s auto-accept leaves no persisted trace:** `_run_validators` runs before `self.backend.transaction()` opens in both methods (before `put_proposal` is ever called), so a rejection means neither the proposal nor any assertion is written — nothing to query afterward, unlike a policy `Reject` (which does persist a `rejected` proposal via `_finalize_non_accepted_decision`). This is accepted as consistent with the direct-write paths' behavior (`assert_literal`/`assert_ref` similarly persist nothing on rejection) rather than fixed to match `Reject`'s auditability — a submitted-then-validator-rejected proposal was never accepted by policy to begin with, so there is no "decision" to record, only a would-be write that didn't happen. A deployment that needs an audit trail for validator rejections specifically has no ready-made answer here (see Consequences).
...
- **A validator that rejects `propose`/`propose_ref`'s auto-accept path (or a direct write) leaves no persisted trace** — no proposal row, no proposal event. See Rationale for why this is accepted rather than fixed to match a policy `Reject`'s auditability. A deployment that needs to audit validator rejections specifically (as opposed to policy rejections) has no ready-made answer here.
- **A `completeness_validators` check for a whole-entity-completeness validator like `RequiredFieldsValidator` receives, per subject, only the *first* op's assertion from that proposal as its `assertion` argument** (`_run_completeness_validators`'s dedup keeps the first occurrence per subject) — correct for `RequiredFieldsValidator` (which only reads `.subject`), but a validator relying on any other field of `assertion` in the completeness path would see an arbitrary, order-dependent one of potentially several ops touching the same subject. `Validator`'s docstring (`plugins/ports.py`) now states this contract explicitly: only `.subject` is meaningful when a Validator is invoked via `completeness_validators`.
