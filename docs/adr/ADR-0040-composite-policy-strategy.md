...
- The other four SPEC §9.2-named strategies (`ConfidenceThreshold`, `TrustLevel`, `SourceRequired`, `RequireReviewByRole`) remain unbuilt — out of scope for KI-061, not newly discovered by this ADR. Tracked as its own follow-up, KI-069 (filed alongside this ADR, since closing KI-061 removed the last KI naming this gap).
