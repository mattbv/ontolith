...
- [ADR-0030](ADR-0030-retract-contradiction-capability-floor.md) — **`retract()` Requires `resolve_contradiction()`'s Capability Floor for Flagged Contradiction Members** (KI-043: retracting a flagged member of an open contradiction now requires `review`/`admin` capability and a non-AI principal, same floor as `resolve_contradiction()`, closing a lower-floor path to the same effective outcome — **Breaking** behavior change, ordinary retraction unaffected)
