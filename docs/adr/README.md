...
- [ADR-0041](ADR-0041-audit-table-immutability-triggers.md) — **`assertion_event`/`proposal_event` Immutability Enforced by SQLite Triggers; No DuckDB Equivalent Exists** (KI-066: `BEFORE UPDATE`/`BEFORE DELETE` triggers on both audit tables make SPEC §17's "MUST NOT be mutable" a store-level guarantee for SQLite, not just a port-surface convention — raises `sqlite3.IntegrityError`, the same class ADR-0011's own DB-layer constraints already use. DuckDB (verified against 1.5.4) has no `CREATE TRIGGER` support and no connection-level access-restriction mechanism to work around it — documented as a currently-unfixable asymmetry, mirroring ADR-0032's own precedent, with `conformance/test_audit_table_immutability.py` pinning both the SQLite guarantee and the DuckDB gap as executable tests rather than only a comment)
