...
- [ADR-0028](ADR-0028-value-type-enforcement-required-deferred.md) — **`value_type` Enforced at Core Write Time; `required` Stays Out of Core** (KI-031, partial: `assert_literal`/`propose` now reject a `value_type` mismatch against the schema's declared `PropertyDef.value_type`; `required` enforcement is deliberately deferred — SPEC §4 assigns it to the validator layer and it's structurally the wrong shape for a per-write core gate — with the gap in the existing `RequiredFieldsValidator` plugin tracked separately as KI-041/KI-042)
