...
**Status as of 2026-08-27** (updated opportunistically, not on every change — treat as a snapshot, verify against `CHANGELOG.md`/`git log` for anything time-sensitive): M0–M2 fully complete. M3's three named exit criteria are all met; the RDF/OWL bridge is now done too (ADR-0036 — schema as OWL ontology plus active-assertion instance data, export only, `rdflib`-backed; no import direction yet). **GraphQL remains the only unstarted item in M3's scope column.** `docs/known-issues.md`'s backlog is empty — KI-001 through KI-051 are all resolved (none blocking, all found during code review rather than reported). M4 has not started.
