...
- **On-disk `format_version`** is independent of SDK version and frozen at 1.0; migrations are reversible or explicitly marked irreversible (SPEC §15). Mechanism shipped 2026-09-20 (ADR-0052) — `format_version` tracked per backend, `ontolith db migrate`/`db status`; the freeze itself (declaring the current number final for 1.0) is a separate, still-open M4 exit criterion.
