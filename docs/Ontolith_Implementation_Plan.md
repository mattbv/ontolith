...
**Status as of 2026-09-10** (updated opportunistically, not on every change — treat as a snapshot, verify against `CHANGELOG.md`/`git log` for anything time-sensitive): M0–M2 fully complete. M3's three named exit criteria are all met, and its full scope column is now complete too: the RDF/OWL bridge (ADR-0036 — schema as OWL ontology plus active-assertion instance data, export only, `rdflib`-backed; no import direction yet) and the GraphQL interface (ADR-0037 — `strawberry`-backed, originally scoped to query/propose/review operations per SPEC §14.3's literal wording, deliberately narrower than REST's own extended write/admin surface; widened once since, to add entity creation — KI-082, ADR-0037's own 2026-09-08 update) both shipped. `docs/known-issues.md`'s backlog holds 4 open items (KI-080 plus KI-093/094/095, three small follow-ups filed while reviewing KI-081; KI-085/KI-087/KI-083/KI-089/KI-082/KI-090/KI-091/KI-088/KI-084/KI-092/KI-086/KI-081, twelve of the thirteen pre-M4 audit KIs and their follow-ups, are resolved — KI-088/KI-084/KI-086/KI-081 were each one of the original nine) plus one carried-forward partial (KI-014, plugin network/filesystem sandboxing, open since M3) — none blocking M3. KI-080 (extend `cardinality="many"` to `time_varying` supersession) needs a genuine design decision revisiting ADR-0017's recorded scope, likely a fresh ADR — the last original audit item, deliberately left for last. KI-093/094/095 (query `.include_flagged()`/`.include_history()` composition with `.min_confidence()`/`.trust_at_least()`; interface-surface parity; a pre-existing retracted-in-`as_of` bitemporal gap) are mechanical or small-design follow-ups from KI-081. KI-001 through KI-079 are otherwise resolved or, for KI-011, a deliberate non-implementation rather than a gap. M4 has not started; the open backlog is intended as input to M4 planning, not a precondition for starting it.
