...
**Status as of 2026-09-14** (updated opportunistically, not on every change — treat as a snapshot, verify against `CHANGELOG.md`/`git log` for anything time-sensitive): M0–M2 fully complete. M3's three named exit criteria are all met, and its full scope column is now complete too: the RDF/OWL bridge (ADR-0036 — schema as OWL ontology plus active-assertion instance data, export only, `rdflib`-backed; no import direction yet) and the GraphQL interface (ADR-0037 — `strawberry`-backed, originally scoped to query/propose/review operations per SPEC §14.3's literal wording, deliberately narrower than REST's own extended write/admin surface; widened once since, to add entity creation — KI-082, ADR-0037's own 2026-09-08 update) both shipped. `docs/known-issues.md`'s backlog holds 3 open items (KI-080, KI-097, KI-098; KI-085/KI-087/KI-083/KI-089/KI-082/KI-090/KI-091/KI-088/KI-084/KI-092/KI-086/KI-081/KI-093/KI-094/KI-096/KI-095 — sixteen of the nineteen pre-M4 audit KIs and their follow-ups — are resolved, leaving KI-080/KI-097/KI-098 open in that same set; KI-088/KI-084/KI-086/KI-081 were each one of the original nine) plus one carried-forward partial (KI-014, plugin network/filesystem sandboxing, open since M3) — none blocking M3. KI-080 (extend `cardinality="many"` to `time_varying` supersession) needs a genuine design decision revisiting ADR-0017's recorded scope, likely a fresh ADR — the last original audit item, deliberately left for last. KI-097 (`as_of` reconstructs `flagged`/`reactivated` from current status rather than point-in-time, found building ADR-0049's KI-095 fix) needs a careful fix, not a design call — the pattern to follow (`assertions()`'s own event-log reconstruction) is already decided. KI-098 (`StorageBackend.assertions()` has no opt-out for ADR-0049's retraction exclusion under `as_of`, found reviewing KI-095) is mechanical, mirroring `include_flagged`'s existing shape on the same method. KI-001 through KI-079 are otherwise resolved or, for KI-011, a deliberate non-implementation rather than a gap. M4 has not started; the open backlog is intended as input to M4 planning, not a precondition for starting it.
