...
**Partial mitigation added:** `PluginRegistry.register()` now logs a `WARNING` when a plugin's manifest declares `network=True` or `filesystem=True`, naming the plugin and stating explicitly that these are not enforced — closes the gap between what the manifest API visually implies (a control) and what's actually true, at the moment (registration) an operator can still decide not to grant it. This is a visibility fix, not an enforcement fix — SPEC §17's MUST itself remains unmet for network/filesystem. "Materializing" `QueryBuilder`'s results eagerly to stop it carrying a backend reference was considered and rejected: it would break the fluent builder API (`.where(...).limit(n).all()`) that's the entire point of `QueryBuilder`, for a property Python's object model can't provide regardless. Real process/subprocess/wasm isolation remains the only actual fix, and remains a materially larger, separate infrastructure project — tracked via the Implementation Plan's existing phasing, not newly deferred here. See ADR-0015's 2026-08-30 update for the full record.
