...
Review found the new blanket redaction turned a pre-existing mislabel in `Ontology.retract()` into an information-destroying one: an unknown `assertion_id` fell through to the backend's generic `set_assertion_status`, whose "no row updated" case raises `StorageError` — previously cosmetic (wrong code, right text reached the caller), now redacted into an opaque "An internal error occurred" everywhere `retract()` is exposed. Fixed at the source: `retract()` now checks `get_assertion(assertion_id) is None` explicitly and raises `NotFoundError` before reaching `set_assertion_status`, matching `flag_contradiction`'s existing precedent — a behavior change on every interface that calls `retract()` (REST now reports 404, not 500), not just MCP. Also added a cross-interface `StorageError` redaction-parity test (`test_cross_interface_error_codes.py`) and corrected an inverted "fails closed" comment on the `isinstance`-based redaction tuple (both MCP and GraphQL).
