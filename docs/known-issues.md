...
**Severity:** Architecture gap — SPEC §11.2's "excluded by default **unless explicitly requested**" has no request path on this one method, post-ADR-0049
**Milestone target:** Backlog
**SPEC reference:** SPEC §11.2 ("Flagged/superseded/retracted assertions are excluded by default; `.include_flagged()`/`.include_history()` opt in"), SPEC §11.4 (`as_of` semantics)
