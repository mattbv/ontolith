...
KI-071 fixed `flag_contradiction()`'s `rationale` being silently dropped on the "extend" branch by accumulating it into `Contradiction.metadata["rationale_history"]`, readable via `Ontology`/the SDK. But `Contradiction.metadata` is not serialized by any of the four shipped interfaces: REST's `ContradictionOut` model, GraphQL's `_contradiction_type`, MCP's `ontolith.flag_contradiction`/query response dicts, and the CLI's `contradiction` sub-app's echo output all omit it entirely. A caller supplying `rationale` through any interface other than the raw SDK has no way to read back what was recorded — structurally the same gap KI-072 closed for the admin-action audit trail (data captured, no read surface), found during KI-071's own review.
