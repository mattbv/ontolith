...
ADR-0050/KI-080 added an optional `supersedes: str | None = None` keyword parameter to `Ontology.assert_literal()`, `assert_ref()`, `propose()`, and `propose_ref()` — deliberately scoped to the Python SDK only, matching this project's established pattern of shipping a capability SDK-first and filing interface parity as its own follow-up (e.g. KI-081 → KI-094/KI-096 for `.include_flagged()`/`.include_history()`). REST's write/propose endpoints, GraphQL's `Mutation.assertLiteral`/`assertRef`/`propose`/`proposeRef` (or their current equivalents), MCP's `propose` tool, and the CLI have no request/argument shape for it at all — a caller on any of those surfaces writing to a `cardinality="many"`, `time_varying` property can only ever get the new coexistence default; explicit replacement of one specific concurrent value is unreachable except from the SDK directly.

### Fix

Not started. Expose `supersedes` on whichever interface a caller first needs it on — REST's request body, GraphQL's mutation input type (camelCased `supersedes` needs no renaming, unlike `includeFlagged`/`includeHistory`), MCP's `propose` tool schema (this project's MCP surface exposes `propose`, not `assert_literal`/`assert_ref` — SPEC §14.4's no-direct-write-tool rule — so only the `propose`/`propose_ref`-side parameter is in scope for MCP, never a direct-write equivalent), and/or the CLI's assert/propose subcommands. Each forwards the string straight through to `Ontology`, which already does all real validation — parity work, not new design.
