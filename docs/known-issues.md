...
- **GraphQL**: `Mutation.propose` is the *only* write-shaped mutation (ADR-0037: GraphQL deliberately has no direct-write mutation, unlike REST) — its `ProposeInput` would need a `supersedes` field, camelCase needing no renaming.
