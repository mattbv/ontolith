...
Added a dated `## Update (2026-09-09, closes KI-084)` section to `docs/adr/ADR-0001-storage-default.md`, elaborating the one-line "single-writer limitation" consequence already on record there into the actual deployment implication: one process should hold the write path per database file; a second process is now safe — for the four write paths named above specifically, not every write path (`create_entity`'s own read-then-write for natural-key uniqueness sits outside any `transaction()` block and isn't reached by this fix; filed separately as KI-092, lower severity since KI-091's `UNIQUE` constraint still catches a race there, just as the wrong error type) — but still serializes behind the first, a throughput consideration rather than a correctness one. Scaling writes across processes is out of scope for the SQLite default and belongs to a server-backed `StorageBackend` instead. The same Update section also documents a cost this fix introduces rather than merely inherits: `self._lock` is acquired before `BEGIN IMMEDIATE`, so a `begin()` genuinely contended by another process now blocks every other call on *this* process — reads included — for up to the full `busy_timeout`, measured directly at ~0.99s against a 1s timeout; previously a deferred `BEGIN` returned near-instantly regardless of contention, so this specific stall is new, not merely newly documented.

Regression-tested directly with two independent `SQLiteBackend` instances on the same file (standing in for two OS processes, since `self._lock`, KI-023's `threading.RLock`, is per-instance and doesn't itself serialize them) — `tests/unit/test_sqlite_backend.py::TestConcurrency::test_begin_blocks_on_cross_process_writer_then_reports_retryable`, `..._releases`, and `test_begin_reports_generic_message_for_non_busy_operational_error` (the last covering the non-`SQLITE_BUSY` fallback branch, via a stand-in connection raising a synthetic `OperationalError` — a real one isn't reliably provokable from Python). Mutation-tested: reverting `BEGIN IMMEDIATE` to `BEGIN` makes the first test fail (contention no longer even attempted at `begin()` time — `DID NOT RAISE`); reverting just the error-code distinguishing logic makes both the first and the fallback test fail on their message assertions; removing the `code & 0xFF` masking (mutated to always match) makes the fallback test fail by wrongly reporting "safe to retry" for a non-busy error. A review round caught that `..._releases` originally passed identically whether or not `begin()` actually contended on anything (its `elapsed`-blocking assertion was missing) — confirmed by reproducing the false-pass directly, then fixed and reconfirmed the fix now fails correctly under the same reverted-`begin()` mutation. A manual timing check (500 `assert_literal` calls, uncontended, after warm-up) confirmed no regression against SPEC's `propose + policy eval + commit: p95 < 50ms` budget: p95 ≈ 0.2ms — `BEGIN IMMEDIATE` costs nothing extra absent actual contention, matching SQLite's documented behavior.
...
## KI-092 — `create_entity()`'s natural-key uniqueness check races a concurrent writer outside any transaction — found while fixing KI-084

**Severity:** Architecture gap — narrower than KI-084 (a race here surfaces as an error, not a silent duplicate, since KI-091's `UNIQUE` constraint still backs it), but the same underlying shape
**Milestone target:** Backlog
**SPEC reference:** SPEC §10 (conflict routing), SPEC §12.1 (SQLite default backend)

### Description

KI-084 fixed the cross-process stale-snapshot race for every write path whose SPEC §10 conflict-routing read runs inside an explicit `transaction()` block: `assert_literal`, `assert_ref`, `propose`, `propose_ref`. `Ontology.create_entity()` (`ontology.py`) has the same read-then-write shape — `_require_unique_natural_key(concept, natural_key)` (a read, added by KI-091) followed by `self.backend.put_entity(entity)` (a write) — but never calls `begin()`/`transaction()` at all; each call is its own implicit autocommit statement. `BEGIN IMMEDIATE`'s fix in `SQLiteBackend.begin()` therefore does not reach this path, confirmed by reading: no `with self.backend.transaction():` wraps either call.

Concretely: two processes racing to create an entity with the same `(namespace, concept, natural_key)` can both pass `_require_unique_natural_key`'s pre-check (reading no conflict yet) before either commits its `put_entity`. Unlike KI-084's original failure mode, this doesn't produce a silent duplicate or an opaque instant failure — KI-091's `UNIQUE(namespace, concept, natural_key)` constraint still catches the loser at the SQLite level, surfacing as the same redacted `StorageError` KI-091 was filed to eliminate for the *non-concurrent* case (a caller who simply supplies a taken key gets the friendly `ValidationError`; a caller who loses this specific race gets the generic redacted one instead, since the constraint fires after `_require_unique_natural_key`'s own check already passed). Lower severity than KI-084 because the failure mode is "wrong error type under a rare race," not "instant unretryable failure" or "silent inconsistency."

### Fix

Decide whether `create_entity()` should gain its own `with self.backend.transaction():` wrapper around the check-then-write (mirroring the four paths KI-084 already covers), which would need `begin()`'s `BEGIN IMMEDIATE` to serialize the race the same way — or whether the existing `UNIQUE`-constraint-catches-it behavior is an acceptable outcome for this specific, narrow, low-frequency race and the fix is instead just mapping the constraint-failure branch to the same `ValidationError` `_require_unique_natural_key` already raises for the non-concurrent case (cheaper, doesn't add a transaction to an otherwise single-statement write path). Worth checking whether `issue_token`/other single-write paths share a version of this question before picking one approach, so it's decided once rather than per call site.
