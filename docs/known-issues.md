...
This is pre-existing SDK behavior, not introduced by KI-082 — but before KI-082, the only caller who could reach it was a human using the CLI directly, who'd typically notice a typo'd concept name immediately. KI-082 exposed `create_entity` on REST/GraphQL/MCP at `propose` tier with no policy evaluation in between (see ADR-0008's 2026-09-08 update for why entity creation has no policy step to catch this at), so an AI agent can now populate the KB with arbitrary undeclared concepts through any of the three governed interfaces, not just a human at a terminal.

### Fix

Add a `_require_known_concept(concept)` check to `create_entity()` — no schema registered for the namespace yet should stay a no-op (matching `_require_known_predicate`'s own precedent), otherwise raise `ValidationError` naming the unknown concept. One check, one call site (`create_entity` is the sole constructor of `Entity`), so no interface-level changes needed beyond the SDK method itself.
