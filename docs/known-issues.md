...
**Found and filed separately while building this fix, not part of this KI's own scope:** **KI-097** — `entities_where()`/`entities_meeting_confidence()`/`entities_meeting_trust()`'s `as_of` branch reconstructs `flagged` from *current* status rather than point-in-time, unlike `StorageBackend.assertions()`'s own already-correct event-log reconstruction for the same status. Same root cause class (current status trusted instead of point-in-time reconstruction) but a distinct transition pair (`flagged`/`reactivated`, a two-way cycle) needing its own, more careful fix — not folded into ADR-0049's simpler one-way `retracted` case.
