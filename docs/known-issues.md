...
New tests in `tests/unit/test_rest.py::TestWriteAssertionRoute`/`TestCreateProposalRoute`, `tests/unit/test_graphql.py::TestProposeMutation`, `tests/unit/test_mcp_server.py::TestProposeTool`, `tests/unit/test_cli.py::TestAssertLiteral` — for each of the eight call sites the change touches (REST's literal/ref branches on both `POST /assertions` and `POST /proposals`; GraphQL's/MCP's literal/ref branches; the CLI's one literal-only command), a forwarding case applying a small `cardinality="many"`, `temporality="time_varying"` schema and confirming `supersedes=<id>` replaces exactly the named prior value (verified via `kb.assertions(status="active")` afterward), mutation-tested (deleting each forwarding line fails exactly its own test and no other). REST and MCP also cover the `time_varying`+`many` relation shape (`assert_ref`/`propose_ref`), not just the literal-property one. One rejected-hint case per interface (REST covers both `POST /assertions` and `POST /proposals`) confirms `VALIDATION_ERROR`/exit code 1 through that interface's own error mapping, not a raw exception — these run against no schema at all (the `temporality != "time_varying"` branch of the scope check), since the SDK-level `cardinality != "many"` branch is already covered directly in `conformance/test_conflict.py`.
