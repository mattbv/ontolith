...
For `create_entity`: decide whether it should gain its own `with self.backend.transaction():` wrapper around the check-then-write (which would need `begin()`'s `BEGIN IMMEDIATE` to serialize the race the same way as the paths KI-084 already covers), or whether the existing `UNIQUE`-constraint-catches-it behavior is an acceptable outcome for this specific, narrow, low-frequency race and the fix is instead just mapping the constraint-failure branch to the same `ValidationError` `_require_unique_natural_key` already raises for the non-concurrent case (cheaper, doesn't add a transaction to an otherwise single-statement write path). `issue_token`/`revoke_token`/`reindex` don't need the same fix — their reads aren't protecting an invariant the write could violate — but are worth a one-line acknowledgment wherever this is resolved, so a future audit doesn't re-discover them as open questions. `propose`/`propose_ref`/`retract`'s reject/require-review path is out of scope here entirely — it's KI-035's decision to revisit, not this KI's.
