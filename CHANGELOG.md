...
- CLI `ontolith schema show [--namespace]` (partially closes KI-038): SPEC §14.2
  ...
  mirroring MCP's `ontolith.schema`/REST's `GET /schema` output (both already
  extended by KI-029 to include relations). `ontolith schema migrate` remains
  unimplemented — schema versioning/migration isn't built anywhere yet, only
  monotonic version numbering via `apply_schema` — so KI-038 stays open for that half.
